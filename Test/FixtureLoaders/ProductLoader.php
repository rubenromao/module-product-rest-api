...
class ProductLoader extends AbstractLoader
{
    ...
    public function createProducts()
    {
        ...
        foreach ($this->productData as $product) {
            ...
            if (isset($product['custom_url_text']) && isset($product['custom_url'])) {
                $productModel->setCustomUrlText($product['custom_url_text']);
                $productModel->setCustomUrl($product['custom_url']);
            }
    ...
    private function getMediaWriter()
    {
        if (null === $this->mediaWriter) {
            $this->mediaWriter = $this->createObject('\Rezolve\Filesystem\Filesystem')->getDirectoryWrite(DirectoryList::MEDIA);
        }
