...
use Rezolve\APISalesV4\Test\Integration\ApiRequests\UpdateOrder\UpdateOrderAbstract;

class TestUnpaidOrdersGetCancelled extends UpdateOrderAbstract
{
    ...
    {
        // Next we need to place an order with the "unpaid" state. We'll also ensure that the JSON which comes back is as
        // we expect.
        $testHelper = $this->getTestHelper();
        $orderId = $this->getCoreOrderId();
        $requestJson = $this->getPlaceOrderRequestJson($orderId);
        $postData = $testHelper->convertJsonToPostData($requestJson);
        $result = $this->placeV3Order('test', '4', $postData);
        $returnedJson = $testHelper->convertReturnedDataToJson($result, JSON_PRETTY_PRINT);
        $expectedJson = $this->getPlaceOrderExpectedJson($orderId);
        $this->assertJsonStringEqualsJsonString($expectedJson, $returnedJson);

        // Next we need to cancel this order using the updateOrder endpoint
        $requestJson = $this->getRequestJson('cancelled');
        $postData = $testHelper->convertJsonToPostData($requestJson);
        $result = $this->makeRequest('chase', '4', $orderId, $postData);
        $returnedJson = $testHelper->convertReturnedDataToJson($result, JSON_PRETTY_PRINT);
        // Check that the order has been cancelled, and not set to "Failed"
        $expectedJson = $this->getExpectedJson('canceled', $orderId);
        $this->assertJsonStringEqualsJsonString($expectedJson, $returnedJson);
    }

    private function getPlaceOrderRequestJson($orderId)
    {
...
JSON;
    ...
    private function getPlaceOrderExpectedJson($orderId)
    {
...
JSON;
    ...
    public function placeV3Order($appId, $merchantId, $postData)
    {
        $serviceInfo = [
            'rest' => [
                'resourcePath' => "/V3/api/mobile/{$appId}/{$merchantId}/order",
