...
class DifferentAddressesAreSavedTest extends PlaceOrderAbstract
{
    /**
    ...
    public function testGetBasketCall()
    {
        ...
        $this->confirmAddressDetails($billingDetails, $billingAddress, 'billing');
        $this->confirmAddressDetails($shippingDetails, $shippingAddress, 'shipping');
    }

    private function confirmAddressDetails(array $expected, OrderAddressInterface $address, $type)
    {
        foreach ($expected as $key => $expectedValue) {
            $method      = 'get' . $key;
            $actualValue = $address->$method();
            $this->assertEquals($expectedValue, $actualValue, "Address details for $key in $type did not match");
        }
    }
...
JSON;
    ...
    public function getExpectedJson($orderId)
    {
