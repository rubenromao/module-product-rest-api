...
class HandleNoCityInBillingAddressTest extends PlaceOrderAbstract
{

    /**
     * @magentoApiDataFixture apiFixtures
     * @magentoApiDataFixture forceCityNotRequired
     */
    public function testGetBasketCall()
    {
        ...
        foreach ($allItems as $item) {
            ...
            if (!array_key_exists($id, $expectedPlacements)) {
                $parentProduct++;
                continue;
            }
            $expectedPlacementId = $expectedPlacements[$id];
            $this->assertEquals($expectedPlacementId, $item->getExtensionAttributes()->getRezolvePlacementId());
        }
        /* Magento stores both the child and parent product  */
        $this->assertEquals(1, $parentProduct);
    }
